/proc/priority_announce(text, title = "", sound = 'sound/ai/attention.ogg', type , sender_override)
	...
	if(type == "Priority")
		...
		if(!sender_override)
			if(title == "")
				GLOB.news_network.SubmitArticle(text, "Head Office Update", "Station Announcements", null)
			else
				GLOB.news_network.SubmitArticle(title + "<br><br>" + text, "Head Office", "Station Announcements", null)
