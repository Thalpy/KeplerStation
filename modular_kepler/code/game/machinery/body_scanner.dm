/obj/machinery/body_scanner
	...
	icon = 'modular_kepler/icons/obj/bodyscanner.dmi'
...
/obj/machinery/body_scanner/Initialize()
	. = ..()
	ui = new /datum/oracle_ui/themed/nano(src, 800, 800, "body_scanner")
		...
		if(carbon_occupant.bodyparts.len)
			for(var/thing in carbon_occupant.bodyparts)
				...
				data["bodyparts"] +=	 "<section>\
										...
										[B.bone_status == BONE_FLAG_BROKEN ? "<span class='average'>FRACTURE</span>" : ""]\
										</section>"
...
/obj/machinery/body_scanner/oui_canview(mob/user)
	if(user.loc == src)
		return FALSE
